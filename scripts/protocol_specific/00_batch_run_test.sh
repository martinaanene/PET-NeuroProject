#!/bin/bash
# Master Batch Processing Script for PET-NeuroProject (TEST VERSION)
# Runs the pipeline for subjects 01 to 02 ONLY
...
# Log file
log_file="batch_processing_log_TEST_$(date +%Y%m%d_%H%M%S).txt"
# Master CSV file for results (Absolute path to ensure sub-scripts find it)
master_csv="$(pwd)/all_subjects_results_TEST.csv"
...
echo "Starting TEST Batch Processing at $(date)" | tee -a "$log_file"
...
echo "TEST Batch Processing Complete. See $log_file for details."
